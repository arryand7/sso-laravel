...
class AuthorizeController extends Controller
{
    ...
    public function authorize(
        ...
        if (! $user->canAccessApplication($application)) {
            return $this->errorResponse($request, 'access_denied', 'User does not have access to this application.', 403, true);
        }

        return $passportController->authorize($serverRequest, $request, $response, $viewResponse);
    }

    protected function errorResponse(Request $request, string $error, string $description, int $status, bool $allowRedirect = false): RedirectResponse|\Illuminate\Http\JsonResponse
    {
        $redirectUri = $request->input('redirect_uri');

        if ($allowRedirect && $redirectUri && filter_var($redirectUri, FILTER_VALIDATE_URL)) {
            $params = [
                'error' => $error,
